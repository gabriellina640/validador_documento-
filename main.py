...
import os
import datetime
import docx # Para gerar o Laudo
...
class AppCorregedoria(ctk.CTk):
    def __init__(self):
        ...
        self.title("Sistema de Validação Institucional")
    ...
    def montar_aba_processar(self):
        lbl_desc = ctk.CTkLabel(self.aba_processar, text="Selecione os arquivos originais para aplicar o registro de validação.", font=ctk.CTkFont(size=14))
    ...
    def montar_aba_auditar(self):
        lbl_desc = ctk.CTkLabel(self.aba_auditar, text="Selecione um documento externo para verificar se ele foi adulterado.", font=ctk.CTkFont(size=14))
        lbl_desc.pack(pady=15)

        btn_verificar = ctk.CTkButton(self.aba_auditar, text="Selecionar Arquivo Suspeito", command=self.verificar_arquivo, fg_color="#FF9800", hover_color="#F57C00", height=40, font=ctk.CTkFont(size=14, weight="bold"))
    ...
    def verificar_arquivo(self):
        ...
        if resultado:
            ...
            self.lbl_res_status.configure(text="❌ DOCUMENTO ADULTERADO OU FALSO", text_color="#F44336")
            self.lbl_res_detalhes.configure(text=f"Aviso: O conteúdo deste arquivo não confere com o banco de dados.\nO documento pode ter sido alterado após a emissão.\nHash Encontrado: {hash_calc[:30]}...")
    ...
    def gerar_laudo_oficial(self):
        ...
        doc = docx.Document()
        doc.add_heading('LAUDO TÉCNICO DE AUDITORIA DE DOCUMENTO', 0)
        ...
        if status == "AUTENTICO":
            ...
            doc.add_paragraph("Atesta-se que a assinatura criptográfica do documento inspecionado confere exatamente com o registro original no banco de dados da instituição.")
            doc.add_paragraph(f"Histórico Original:\n- Processado originalmente em: {info[1]}\n- Pelo operador: {info[2]}")
        else:
            p_resultado.add_run("DOCUMENTO ADULTERADO / NÃO RECONHECIDO.").bold = True
            doc.add_paragraph("Atesta-se que a assinatura criptográfica do documento inspecionado NÃO confere com nenhum registro válido no banco de dados. O arquivo sofreu alterações de conteúdo após sua possível emissão original ou foi forjado.")
